...
test("a player identity scans for ten seconds then materializes for ten seconds", async ({ browser }) => {
  ...
  await player.getByLabel("Qui es-tu ?").click()
  await player.getByRole("option", { name: "Autre invité" }).click()
  ...
  await expect(player.getByText("Identité confirmée")).toHaveCount(0)

  const image = player.getByTestId("totem-materializing-image")
  await expect(image).toHaveAttribute("src", "/players/anonyme-poisson-clown.png")
  ...
  await expect(player.getByText("Identité confirmée")).toBeVisible({ timeout: 12_000 })
  await expect(player.getByTestId("totem-reveal-image")).toHaveAttribute("alt", "Ariel")
