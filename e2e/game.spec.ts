...
test("four players form teams and start the tournament", async ({ browser }) => {
  ...
  await expect(host.getByLabel("Estimation en kg")).toHaveCount(0)
  await expect(guests[0].getByLabel("Estimation en kg")).toBeVisible()
  await guests[0].getByLabel("Estimation en kg").fill("0,09")
  ...
  await expect(guests[0].getByLabel("Estimation en kg")).toBeVisible()
  await guests[0].getByLabel("Estimation en kg").fill("270")
  await guests[0].getByRole("button", { name: "Valider la réponse" }).click()
  await host.getByRole("button", { name: "Révéler maintenant" }).click()
  await host.getByRole("button", { name: "Manche suivante" }).click()
  await expect(guests[0].getByLabel("Estimation en kg")).toBeVisible()
  await guests[0].getByLabel("Estimation en kg").fill("150000")
  await guests[0].getByRole("button", { name: "Valider la réponse" }).click()
  await host.getByRole("button", { name: "Révéler maintenant" }).click()
